...
/**
  * Created by nappannda on 2018/01/16.
  */
abstract class Combinator {
  ...
  def rep[T](parser: => Parser[T]): Parser[List[T]] = input => {
    ...
    def repeatRec(input: String): (List[T], String) =
      parser(input) match {
  ...
  def rep1sep[T](parser: => Parser[T], sep: Parser[String]): Parser[List[T]] = parser ~ rep(sep ~> parser) ^^ { t => t._1 :: t._2 }

  def success[T](value: T): Parser[T] = input => Success(value, input)

  def repsep[T](parser: => Parser[T], sep: Parser[String]): Parser[List[T]] = rep1sep(parser, sep) | success(List())
